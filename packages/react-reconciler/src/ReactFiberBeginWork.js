...
export function reconcileChildren(
  ...
  if (current === null) {
    ...
    workInProgress[6] = mountChildFibers(
      workInProgress,
      null,
      nextChildren,
      renderLanes,
    ...
    // If the current[6] is the same as the work in progress, it means that
    // we haven't yet started any work on these children. Therefore, we use
    // the clone algorithm to create a copy of all the current[6]ren.

    // If we had any progressed work already, that is invalid at this point so
    // let's throw it out.
    workInProgress[6] = reconcileChildFibers(
      workInProgress,
      current[6],
      nextChildren,
      renderLanes,
...
function forceUnmountCurrentAndReconcile(
  ...
  // effect of all current[6]ren being unmounted; even if the type and key
  // are the same, the old child is unmounted and a new child is created.
  //
  // To do this, we're going to go through the reconcile algorithm twice. In
  // the first pass, we schedule a deletion for all the current[6]ren by
  // passing null.
  workInProgress[6] = reconcileChildFibers(
    workInProgress,
    current[6],
  ...
  // pass null in place of where we usually pass the current[6] set. This has
...
function updateForwardRef(
  ...
  workInProgress[17] |= PerformedWork;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress[6];
}
...
function updateMemoComponent(
  ...
  if (current === null) {
    ...
    if (
      ...
      workInProgress[0] = SimpleMemoComponent;
      workInProgress[3] = resolvedType;
      if (__DEV__) {
    ...
    child[9] = workInProgress[9];
    child[5] = workInProgress;
    workInProgress[6] = child;
    return child;
  }
  const currentChild = ((current[6]: any): Fiber); // This is always exactly one child
  ...
  if (!hasScheduledUpdateOrContext) {
    ...
    // unlike current[12] which will be the unresolved ones.
    const prevProps = currentChild[12];
    // Default to shallow comparison
    let compare = Component.compare;
    compare = compare !== null ? compare : shallowEqual;
    if (compare(prevProps, nextProps) && current[9] === workInProgress[9]) {
      return bailoutOnAlreadyFinishedWork(current, workInProgress, renderLanes);
    }
  }
  // React DevTools reads this flag.
...
function updateSimpleMemoComponent(
  ...
  // We'll need to figure out if this is fine or can cause issues.
  if (current !== null) {
    const prevProps = current[12];
    if (
      shallowEqual(prevProps, nextProps) &&
      current[9] === workInProgress[9] &&
      // Prevent bailout if the implementation changed due to hot reload.
      (__DEV__ ? workInProgress[3] === current[3] : true)
    ) {
      ...
      workInProgress[11] = nextProps = prevProps;
      ...
      } else if ((current[17] & ForceUpdateForLegacySuspense) !== NoFlags) {
...
function updateOffscreenComponent(
  ...
  const nextProps: OffscreenProps = workInProgress[11];
  const nextChildren = nextProps.children;
  const nextIsDetached =
    (workInProgress[4]._pendingVisibility & OffscreenDetached) !== 0;
  ...
  if (
    ...
    const didSuspend = (workInProgress[17] & DidCapture) !== NoFlags;
    if (didSuspend) {
      ...
      if (current !== null) {
        // Reset to the current[6]ren
        let currentChild = (workInProgress[6] = current[6]);
        ...
        while (currentChild !== null) {
          currentChildLanes = mergeLanes(
            mergeLanes(currentChildLanes, currentChild[20]),
            currentChild[21],
          );
        ...
        workInProgress[21] = remainingChildLanes;
      } else {
    ...
    if (
      !disableLegacyMode &&
      (workInProgress[16] & ConcurrentMode) === NoMode
    ) {
      ...
      workInProgress[20] = workInProgress[21] =
    ...
    if (prevState !== null) {
      ...
      if (enableTransitionTracing) {
        ...
        const instance: OffscreenInstance | null = workInProgress[4];
...
function updateCacheComponent(
  ...
  if (current === null) {
    ...
    workInProgress[14] = initialState;
    initializeUpdateQueue(workInProgress);
    pushCacheProvider(workInProgress, freshCache);
  } else {
    // Check for updates
    if (includesSomeLane(current[20], renderLanes)) {
      cloneUpdateQueue(current, workInProgress);
      processUpdateQueue(workInProgress, null, null, renderLanes);
      suspendIfUpdateReadFromEntangledAsyncAction();
    }
    const prevState: CacheComponentState = current[14];
    const nextState: CacheComponentState = workInProgress[14];
  ...
  const nextChildren = workInProgress[11].children;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress[6];
}
...
function updateTracingMarkerComponent(
  ...
  if (current === null) {
    ...
    if (currentTransitions !== null) {
      const markerInstance: TracingMarkerInstance = {
        ...
        name: workInProgress[11].name,
        aborts: null,
      };
      workInProgress[4] = markerInstance;
      ...
      // complete callback.
      workInProgress[17] |= Passive;
    }
  } else {
    if (__DEV__) {
      if (current[12].name !== workInProgress[11].name) {
        console.error(
  ...
  const instance: TracingMarkerInstance | null = workInProgress[4];
  if (instance !== null) {
...
function updateFragment(
  ...
  const nextChildren = workInProgress[11];
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress[6];
}
...
function updateProfiler(
  ...
  if (enableProfilerTimer) {
    ...
    if (enableProfilerCommitHooks) {
      ...
      const stateNode = workInProgress[4];
      stateNode.effectDuration = 0;
      stateNode.passiveEffectDuration = 0;
    }
  }
  const nextProps = workInProgress[11];
  const nextChildren = nextProps.children;
  reconcileChildren(current, workInProgress, nextChildren, renderLanes);
  return workInProgress[6];
}
...
function markRef(current: Fiber | null, workInProgress: Fiber) {
  // TODO: Check props.ref instead of fiber[9] when enableRefAsProp is on.
  const ref = workInProgress[9];
  if (ref === null) {
    if (current !== null && current[9] !== null) {
      // Schedule a Ref effect
      workInProgress[17] |= Ref | RefStatic;
    }
  } else {
    ...
    if (current === null || current[9] !== ref) {
      if (!disableStringRefs && current !== null) {
        const oldRef = current[9];
        const newRef = ref;
        if (
...
function mountIncompleteFunctionComponent(
  ...
  workInProgress[0] = FunctionComponent;
...
function updateFunctionComponent(
  ...
  if (__DEV__) {
    ...
    if (current === null) {
      ...
      validateFunctionComponentInDev(workInProgress, workInProgress[3]);
...
function updateClassComponent(
  ...
  if (__DEV__) {
    ...
    switch (shouldError(workInProgress)) {
      ...
      case true: {
        workInProgress[17] |= DidCapture;
        workInProgress[17] |= ShouldCapture;
        // eslint-disable-next-line react-internal/prod-error-codes
        const error = new Error('Simulated error coming from DevTools');
        const lane = pickArbitraryLane(renderLanes);
        workInProgress[20] = mergeLanes(workInProgress[20], lane);
...
function finishClassComponent(
  ...
  const didCaptureError = (workInProgress[17] & DidCapture) !== NoFlags;
  ...
  workInProgress[14] = instance.state;
...
function pushHostRootContext(workInProgress: Fiber) {
  const root = (workInProgress[4]: FiberRoot);
...
function updateHostRoot(
  ...
  const nextState: RootState = workInProgress[14];
  ...
  if (supportsHydration && prevState.isDehydrated) {
    ...
    const updateQueue: UpdateQueue<RootState> =
      (workInProgress[13]: any);
    // `baseState` can always be the last state because the root doesn't
    // have reducer functions so it doesn't need rebasing.
    updateQueue.baseState = overrideState;
    workInProgress[14] = overrideState;

    if (workInProgress[17] & ForceClientRender) {
      ...
      while (node) {
        ...
        node[17] = (node[17] & ~Placement) | Hydrating;
        node = node[7];
      }
    }
  } else {
...
function mountHostRootWithoutHydrating(
  ...
  workInProgress[17] |= ForceClientRender;
...
function updateHostComponent(
  ...
  const nextProps = workInProgress[11];
  const prevProps = current !== null ? current[12] : null;
  ...
  if (isDirectTextChild) {
    ...
    workInProgress[17] |= ContentReset;
  }

  if (enableAsyncActions) {
    const memoizedState = workInProgress[14];
    if (memoizedState !== null) {
      ...
      if (enableLazyContextPropagation) {
        ...
        if (didReceiveUpdate) {
          if (current !== null) {
            const oldStateHook: Hook = current[14];
...
function updateHostHoistable(
  ...
  markRef(current, workInProgress);
  const currentProps = current === null ? null : current[12];
  const resource = (workInProgress[14] = getResource(
    workInProgress[3],
    currentProps,
    workInProgress[11],
  ));
  if (current === null) {
    if (!getIsHydrating() && resource === null) {
      ...
      workInProgress[4] = createHoistableInstance(
        workInProgress[3],
        workInProgress[11],
        getRootHostContainer(),
        workInProgress,
  ...
  if (typeof Component === 'function') {
    ...
    if ($$typeof === REACT_FORWARD_REF_TYPE) {
      ...
      const resolvedProps = disableDefaultPropsExceptForClasses
        ? props
        : resolveDefaultPropsOnNonClassComponent(Component, props);
      workInProgress[0] = MemoComponent;
...
function validateFunctionComponentInDev(workInProgress: Fiber, Component: any) {
  if (__DEV__) {
    ...
    if (!enableRefAsProp && workInProgress[9] !== null) {
      let info = '';
      const componentName = getComponentNameFromType(Component) || 'Unknown';
...
function shouldRemainOnFallback(
  ...
  if (current !== null) {
    const suspenseState: SuspenseState = current[14];
...
function getRemainingWorkInPrimaryTree(
  ...
  let remainingLanes =
    current !== null ? removeLanes(current[21], renderLanes) : NoLanes;
...
function updateSuspenseComponent(
  ...
  const didPrimaryChildrenDefer = (workInProgress[17] & DidDefer) !== NoFlags;
  workInProgress[17] &= ~DidDefer;
  ...
  if (current === null) {
    ...
    if (showFallback) {
      ...
      const primaryChildFragment: Fiber = (workInProgress[6]: any);
      primaryChildFragment[14] =
        mountSuspenseOffscreenState(renderLanes);
      primaryChildFragment[21] = getRemainingWorkInPrimaryTree(
        current,
        didPrimaryChildrenDefer,
        renderLanes,
      );
      workInProgress[14] = SUSPENDED_MARKER;
      if (enableTransitionTracing) {
        const currentTransitions = getPendingTransitions();
        if (currentTransitions !== null) {
          const parentMarkerInstances = getMarkerInstances();
          const offscreenQueue: OffscreenQueue | null =
            (primaryChildFragment[13]: any);
          if (offscreenQueue === null) {
            ...
            primaryChildFragment[13] = newOffscreenQueue;
          } else {
      ...
      workInProgress[20] = SomeRetryLane;
      return fallbackFragment;
    } else {
      pushPrimaryTreeSuspenseHandler(workInProgress);
    ...
    // Special path for hydration
    const prevState: null | SuspenseState = current[14];
    if (prevState !== null) {
    ...
    if (showFallback) {
      ...
      const primaryChildFragment: Fiber = (workInProgress[6]: any);
      const prevOffscreenState: OffscreenState | null = (current[6]: any)
        .memoizedState;
      primaryChildFragment[14] =
...
function mountSuspensePrimaryChildren(
  ...
  primaryChildFragment[5] = workInProgress;
  workInProgress[6] = primaryChildFragment;
  return primaryChildFragment;
}
...
function mountSuspenseFallbackChildren(
  ...
  const mode = workInProgress[16];
  const progressedPrimaryFragment: Fiber | null = workInProgress[6];
  ...
  if (
    ...
    primaryChildFragment[21] = NoLanes;
    primaryChildFragment[11] = primaryChildProps;

    if (enableProfilerTimer && workInProgress[16] & ProfileMode) {
      // Reset the durations from the first pass so they aren't included in the
      // final amounts. This seems counterintuitive, since we're intentionally
      // not measuring part of the render phase, but this makes it match what we
      // do in Concurrent Mode.
      primaryChildFragment[23] = 0;
      primaryChildFragment[24] = -1;
      primaryChildFragment[25] = 0;
      primaryChildFragment[26] = 0;
    }
  ...
  primaryChildFragment[5] = workInProgress;
  fallbackChildFragment[5] = workInProgress;
  primaryChildFragment[7] = fallbackChildFragment;
  workInProgress[6] = primaryChildFragment;
  return fallbackChildFragment;
}
...
function updateSuspensePrimaryChildren(
  ...
  const currentPrimaryChildFragment: Fiber = (current[6]: any);
  const currentFallbackChildFragment: Fiber | null =
    currentPrimaryChildFragment[7];
  ...
  if (!disableLegacyMode && (workInProgress[16] & ConcurrentMode) === NoMode) {
    primaryChildFragment[20] = renderLanes;
  }
  primaryChildFragment[5] = workInProgress;
  primaryChildFragment[7] = null;
  if (currentFallbackChildFragment !== null) {
    // Delete the fallback child fragment
    const deletions = workInProgress[19];
    if (deletions === null) {
      workInProgress[19] = [currentFallbackChildFragment];
      workInProgress[17] |= ChildDeletion;
    } else {
...
function updateSuspenseFallbackChildren(
  ...
  if (
    ...
    // only codepath.)
    workInProgress[6] !== currentPrimaryChildFragment
  ) {
    const progressedPrimaryFragment: Fiber = (workInProgress[6]: any);
    primaryChildFragment = progressedPrimaryFragment;
    primaryChildFragment[21] = NoLanes;
    primaryChildFragment[11] = primaryChildProps;
    ...
    if (enableProfilerTimer && workInProgress[16] & ProfileMode) {
      ...
      primaryChildFragment[23] = 0;
      primaryChildFragment[24] = -1;
      primaryChildFragment[25] =
        currentPrimaryChildFragment[25];
      primaryChildFragment[26] =
        currentPrimaryChildFragment[26];
    }
    ...
    primaryChildFragment[18] =
      currentPrimaryChildFragment[18] & StaticMask;
  }
  let fallbackChildFragment;
  if (currentFallbackChildFragment !== null) {
    ...
    fallbackChildFragment[17] |= Placement;
  }
...
function retrySuspenseComponentWithoutHydrating(
  ...
  reconcileChildFibers(workInProgress, current[6], null, renderLanes);
  ...
  // Needs a placement effect because the parent (the Suspense boundary) already
  // mounted but this is a new fiber.
  primaryChildFragment[17] |= Placement;
  workInProgress[14] = null;
...
function mountSuspenseFallbackAfterRetryWithoutHydrating(
  ...
  const fiberMode = workInProgress[16];
  const primaryChildProps: OffscreenProps = {
    mode: 'visible',
    children: primaryChildren,
  };
  ...
  if (disableLegacyMode || (workInProgress[16] & ConcurrentMode) !== NoMode) {
    ...
    // deletion. We need to reconcile to delete the current[6].
    reconcileChildFibers(workInProgress, current[6], null, renderLanes);
  }
...
function mountDehydratedSuspenseComponent(
  ...
  if (isSuspenseInstanceFallback(suspenseInstance)) {
    ...
    workInProgress[20] = laneToLanes(DefaultHydrationLane);
  } else {
    // We'll continue hydrating the rest at offscreen priority since we'll already
    // be showing the right content coming from the server, it is no rush.
    workInProgress[20] = laneToLanes(OffscreenLane);
  }
  return null;
}
...
function updateDehydratedSuspenseComponent(
  ...
  if (!didSuspend) {
    ...
    const hasContextChanged = includesSomeLane(renderLanes, current[21]);
    if (didReceiveUpdate || hasContextChanged) {
      ...
      // inserted into the React tree here. It just happens to not need DOM
      // mutations because it already exists.
      primaryChildFragment[17] |= Hydrating;
      return primaryChildFragment;
    }
  } else {
    ...
    } else if ((workInProgress[14]: null | SuspenseState) !== null) {
...
function scheduleSuspenseWorkOnFiber(
  ...
  scheduleContextWorkOnParentPath(fiber[5], renderLanes, propagationRoot);
}
...
function propagateSuspenseContextChange(
  ...
  while (node !== null) {
    ...
    } else if (node[0] === SuspenseListComponent) {
    ...
    while (node[7] === null) {
      // $FlowFixMe[incompatible-use] found when upgrading Flow
      if (node[5] === null || node[5] === workInProgress) {
        return;
      }
      node = node[5];
    }
...
function updateSuspenseListComponent(
  ...
  if (shouldForceFallback) {
    ...
    const didSuspendBefore =
      current !== null && (current[17] & DidCapture) !== NoFlags;
    if (didSuspendBefore) {
  ...
  if (!disableLegacyMode && (workInProgress[16] & ConcurrentMode) === NoMode) {
    ...
    switch (revealOrder) {
      case 'forwards': {
        const lastContentRow = findLastContentRow(workInProgress[6]);
        let tail;
        if (lastContentRow === null) {
          // The whole list is part of the tail.
          // TODO: We could fast path by just rendering the tail now.
          tail = workInProgress[6];
          workInProgress[6] = null;
        } else {
          // Disconnect the tail rows after the content row.
          // We're going to render them separately later.
          tail = lastContentRow[7];
          lastContentRow[7] = null;
        }
      ...
      case 'backwards': {
        ...
        // TODO: If workInProgress[6] is null, we can continue on the tail immediately.
        initSuspenseListRenderState(
          workInProgress,
          true, // isBackwards
          tail,
...
function updatePortalComponent(
  ...
  pushHostContainer(workInProgress, workInProgress[4].containerInfo);
  const nextChildren = workInProgress[11];
  if (current === null) {
...
function updateContextProvider(
  ...
  const newProps = workInProgress[11];
  const oldProps = workInProgress[12];
...
function updateContextConsumer(
  ...
  let context: ReactContext<any>;
  if (enableRenderableContext) {
    const consumerType: ReactConsumerType<any> = workInProgress[3];
...
function resetSuspendedCurrentOnMountInLegacyMode(
  ...
  if (!disableLegacyMode && (workInProgress[16] & ConcurrentMode) === NoMode) {
    if (current !== null) {
      ...
      current[22] = null;
      workInProgress[22] = null;
      // Since this is conceptually a new fiber, schedule a Placement effect
      workInProgress[17] |= Placement;
    }
  }
}
...
function bailoutOnAlreadyFinishedWork(
  ...
  markSkippedUpdateLanes(workInProgress[20]);

  // Check if the children have any pending work.
  if (!includesSomeLane(renderLanes, workInProgress[21])) {
...
function remountFiber(
  ...
  if (__DEV__) {
    const returnFiber = oldWorkInProgress[5];
    if (returnFiber === null) {
      // eslint-disable-next-line react-internal/prod-error-codes
    ...
    // It will get deleted.
    current[22] = null;
    oldWorkInProgress[22] = null;

    // Connect to the new tree.
    newWorkInProgress[8] = oldWorkInProgress[8];
    newWorkInProgress[7] = oldWorkInProgress[7];
    newWorkInProgress[5] = oldWorkInProgress[5];
    newWorkInProgress[9] = oldWorkInProgress[9];

    if (__DEV__) {
      newWorkInProgress[27] = oldWorkInProgress[27];
    }

    // Replace the child[7] pointers above it.
    if (oldWorkInProgress === returnFiber[6]) {
      returnFiber[6] = newWorkInProgress;
    } else {
      let prevSibling = returnFiber[6];
      if (prevSibling === null) {
        // eslint-disable-next-line react-internal/prod-error-codes
        throw new Error('Expected parent to have a child.');
      }
      // $FlowFixMe[incompatible-use] found when upgrading Flow
      while (prevSibling[7] !== oldWorkInProgress) {
        // $FlowFixMe[incompatible-use] found when upgrading Flow
        prevSibling = prevSibling[7];
        if (prevSibling === null) {
          // eslint-disable-next-line react-internal/prod-error-codes
      ...
      // $FlowFixMe[incompatible-use] found when upgrading Flow
      prevSibling[7] = newWorkInProgress;
    }
    ...
    if (deletions === null) {
      returnFiber[19] = [current];
      returnFiber[17] |= ChildDeletion;
    } else {
      deletions.push(current);
    }

    newWorkInProgress[17] |= Placement;
...
function attemptEarlyBailoutIfNoScheduledUpdate(
  ...
  switch (workInProgress[0]) {
    case HostRoot:
      ...
      if (enableCache) {
        const cache: Cache = current[14].cache;
    ...
    case ContextProvider: {
      const newValue = workInProgress[12].value;
      let context: ReactContext<any>;
      if (enableRenderableContext) {
        context = workInProgress[3];
      } else {
    ...
    case SuspenseComponent: {
      const state: SuspenseState | null = workInProgress[14];
      if (state !== null) {
        ...
        // child fragment.
        const primaryChildFragment: Fiber = (workInProgress[6]: any);
        const primaryChildLanes = primaryChildFragment[21];
    ...
    case SuspenseListComponent: {
      const didSuspendBefore = (current[17] & DidCapture) !== NoFlags;
      ...
      if (enableLazyContextPropagation && !hasChildWork) {
        ...
        hasChildWork = includesSomeLane(renderLanes, workInProgress[21]);
      }
      ...
      const renderState = workInProgress[14];
      if (renderState !== null) {
...
function beginWork(
  ...
  if (__DEV__) {
    if (workInProgress[30] && current !== null) {
  ...
  if (current !== null) {
    ...
    if (
      ...
      (__DEV__ ? workInProgress[3] !== current[3] : false)
    ) {
      ...
      if (
        ...
        (workInProgress[17] & DidCapture) === NoFlags
      ) {
      ...
      if ((current[17] & ForceUpdateForLegacySuspense) !== NoFlags) {
        // This is a special case that only exists for legacy mode.
        // See https://github.com/facebook/react/pull/19216.
        didReceiveUpdate = true;
      } else {
    ...
    if (getIsHydrating() && isForkedChild(workInProgress)) {
      ...
      const slotIndex = workInProgress[8];
  ...
  switch (workInProgress[0]) {
    case LazyComponent: {
      const elementType = workInProgress[2];
    ...
    case FunctionComponent: {
      const Component = workInProgress[3];
      const unresolvedProps = workInProgress[11];
      const resolvedProps =
        disableDefaultPropsExceptForClasses ||
        workInProgress[2] === Component
          ? unresolvedProps
  ...
  throw new Error(
    `Unknown unit of work tag (${workInProgress[0]}). This error is likely caused by a bug in ` +
      'React. Please file an issue.',
  );
}
