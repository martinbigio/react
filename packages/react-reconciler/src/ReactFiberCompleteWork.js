...
function doesRequireClone(current: null | Fiber, completedWork: Fiber) {
  const didBailout = current !== null && current[6] === completedWork[6];
  if (didBailout) {
    return false;
  }

  if ((completedWork[17] & ChildDeletion) !== NoFlags) {
    return true;
  }

  // TODO: If we move the `doesRequireClone` call after `bubbleProperties`
  // then we only have to check the `completedWork[18]`.
  let child = completedWork[6];
  while (child !== null) {
    if (
      (child[17] & MutationMask) !== NoFlags ||
      (child[18] & MutationMask) !== NoFlags
    ) {
      return true;
    }
    child = child[7];
...
function appendAllChildren(
  ...
  if (supportsMutation) {
    ...
    let node = workInProgress[6];
    while (node !== null) {
      if (node[0] === HostComponent || node[0] === HostText) {
        appendInitialChild(parent, node[4]);
      } else if (
        node[0] === HostPortal ||
...
function updateHostComponent(
  ...
  if (supportsMutation) {
    ...
    const currentInstance = current[4];
    const oldProps = current[12];
    ...
    if (!requiresClone && oldProps === newProps) {
      ...
      workInProgress[4] = currentInstance;
      return;
    }
...
function preloadInstanceAndSuspendIfNeeded(
  ...
  if (!maySuspendCommit(type, props)) {
    ...
    workInProgress[17] &= ~MaySuspendCommit;
    return;
  }
  ...
  if (!includesOnlyNonUrgentLanes(rootRenderLanes)) {
    ...
    if (!isReady) {
      if (shouldRemainOnPreviousScreen()) {
        ...
        workInProgress[17] |= ShouldSuspendCommit;
      } else {
...
function scheduleRetryEffect(
  ...
  if (wakeables !== null) {
    ...
    if (workInProgress[17] & ScheduleRetry) {
      const retryLane =
        ...
        // and retry lanes.
        workInProgress[0] !== OffscreenComponent
          ? claimNextRetryLane()
          : OffscreenLane;
      workInProgress[20] = mergeLanes(workInProgress[20], retryLane);
    }
  }
}
...
function updateHostText(
  ...
  if (supportsMutation) {
    ...
    if (oldText !== newText) {
      ...
      const currentHostContext = getHostContext();
      workInProgress[4] = createTextInstance(
        newText,
...
function bubbleProperties(completedWork: Fiber) {
  const didBailout =
    completedWork[22] !== null &&
    completedWork[22][6] === completedWork[6];
  ...
  if (!didBailout) {
    ...
    if (enableProfilerTimer && (completedWork[16] & ProfileMode) !== NoMode) {
      ...
      // profiler durations.
      let actualDuration = completedWork[23];
      let treeBaseDuration = ((completedWork[25]: any): number);

      let child = completedWork[6];
      while (child !== null) {
        newChildLanes = mergeLanes(
          newChildLanes,
          mergeLanes(child[20], child[21]),
        );

        subtreeFlags |= child[18];
        subtreeFlags |= child[17];
        ...
        // $FlowFixMe[unsafe-addition] addition with possible null/undefined value
        treeBaseDuration += child[26];
        child = child[7];
      }

      completedWork[23] = actualDuration;
      completedWork[26] = treeBaseDuration;
    } else {
      let child = completedWork[6];
      while (child !== null) {
        newChildLanes = mergeLanes(
        ...
        child[5] = completedWork;

        child = child[7];
      }
    }

    completedWork[18] |= subtreeFlags;
  } else {
    // Bubble up the earliest expiration time.
    if (enableProfilerTimer && (completedWork[16] & ProfileMode) !== NoMode) {
      // In profiling mode, resetChildExpirationTime is also used to reset
      ...
      while (child !== null) {
        ...
        // ignore them.
        subtreeFlags |= child[18] & StaticMask;
        subtreeFlags |= child[17] & StaticMask;
  ...
  completedWork[21] = newChildLanes;
...
function completeDehydratedSuspenseBoundary(
  ...
  if (nextState !== null && nextState.dehydrated !== null) {
    ...
    if (current === null) {
      ...
      resetHydrationState();
      if ((workInProgress[17] & DidCapture) === NoFlags) {
...
function completeWork(
  ...
  switch (workInProgress[0]) {
    ...
    case HostRoot: {
      const fiberRoot = (workInProgress[4]: FiberRoot);
      ...
      if (enableCache) {
        ...
        if (current !== null) {
          previousCache = current[14].cache;
        }
      ...
      if (current === null || current[6] === null) {
        ...
        if (wasHydrated) {
          ...
          if (current !== null) {
            const prevState: RootState = current[14];
            if (
              // Check if this is a client root
              !prevState.isDehydrated ||
              // Check if we reverted to client rendering (e.g. due to an error)
              (workInProgress[17] & ForceClientRender) !== NoFlags
            ) {
              // Schedule an effect to clear this container at the start of the
              // next commit. This handles the case of React rendering into a
              // container with previous children. It's also safe to do for
              // updates too, because current[6] would only be null if the
      ...
      bubbleProperties(workInProgress);
      if (enableTransitionTracing) {
        if ((workInProgress[18] & Visibility) !== NoFlags) {
    ...
    case HostHoistable: {
      if (supportsResources) {
        ...
        const nextResource: Resource | null = workInProgress[14];
        if (current === null) {
          ...
          // We are updating.
          const currentResource = current[14];
    ...
    case HostSingleton: {
      if (supportsSingletons) {
        ...
        const rootContainerInstance = getRootHostContainer();
        const type = workInProgress[3];
        if (current !== null && workInProgress[4] != null) {
          if (supportsMutation) {
            const oldProps = current[12];
            if (oldProps !== newProps) {
              markUpdate(workInProgress);
    ...
    case HostText: {
      ...
      if (current && workInProgress[4] != null) {
        const oldText = current[12];
    ...
    case SuspenseComponent: {
      const nextState: null | SuspenseState = workInProgress[14];
      ...
      const prevDidTimeout =
        current !== null &&
        (current[14]: null | SuspenseState) !== null;

      if (enableCache && nextDidTimeout) {
        const offscreenFiber: Fiber = (workInProgress[6]: any);
        let previousCache: Cache | null = null;
        if (
          offscreenFiber[22] !== null &&
          offscreenFiber[22][14] !== null &&
          offscreenFiber[22][14].cachePool !== null
        ) {
          previousCache = offscreenFiber[22][14].cachePool.pool;
        }
        let cache: Cache | null = null;
        if (
          offscreenFiber[14] !== null &&
          offscreenFiber[14].cachePool !== null
        ) {
          cache = offscreenFiber[14].cachePool.pool;
        }
        if (cache !== previousCache) {
          // Run passive effects to retain/release the cache.
          offscreenFiber[17] |= Passive;
        }
      }
      ...
      if (nextDidTimeout !== prevDidTimeout) {
        ...
        if (nextDidTimeout) {
          const offscreenFiber: Fiber = (workInProgress[6]: any);
          offscreenFiber[17] |= Visibility;
        }
      }

      const retryQueue: RetryQueue | null = (workInProgress[13]: any);
      scheduleRetryEffect(workInProgress, retryQueue);

      if (
        enableSuspenseCallback &&
        workInProgress[13] !== null &&
        workInProgress[12].suspenseCallback != null
      ) {
    ...
    case HostPortal:
      ...
      updateHostContainer(current, workInProgress);
      if (current === null) {
        preparePortalMount(workInProgress[4].containerInfo);
      }
      bubbleProperties(workInProgress);
      return null;
    ...
    case SuspenseListComponent: {
      ...
      let didSuspendAlready = (workInProgress[17] & DidCapture) !== NoFlags;
      ...
      if (renderedTail === null) {
        ...
        if (!didSuspendAlready) {
          ...
          if (suspended !== null) {
            ...
            const retryQueue: RetryQueue | null = (suspended[13]: any);
            workInProgress[13] = retryQueue;
            scheduleRetryEffect(workInProgress, retryQueue);
        ...
        if (renderState.isBackwards) {
          ...
          renderedTail[7] = workInProgress[6];
          workInProgress[6] = renderedTail;
        } else {
          const previousSibling = renderState.last;
          if (previousSibling !== null) {
            previousSibling[7] = renderedTail;
          } else {
            workInProgress[6] = renderedTail;
          }
    ...
    case ScopeComponent: {
      if (enableScopeAPI) {
        if (current === null) {
          ...
          workInProgress[4] = scopeInstance;
          prepareScopeUpdate(scopeInstance, workInProgress);
          if (workInProgress[9] !== null) {
    ...
    case LegacyHiddenComponent: {
      ...
      const nextState: OffscreenState | null = workInProgress[14];
      const nextIsHidden = nextState !== null;

      // Schedule a Visibility effect if the visibility has changed
      if (enableLegacyHidden && workInProgress[0] === LegacyHiddenComponent) {
        // LegacyHidden doesn't do any hiding — it only pre-renders.
      } else {
        if (current !== null) {
          const prevState: OffscreenState | null = current[14];
      ...
      if (
        ...
        if (
          ...
          (workInProgress[17] & DidCapture) === NoLanes
        ) {
          bubbleProperties(workInProgress);
          ...
          if (
            (!enableLegacyHidden ||
              workInProgress[0] !== LegacyHiddenComponent) &&
            workInProgress[18] & (Placement | Update)
          ) {
            workInProgress[17] |= Visibility;
          }
        }
      ...
      if (enableCache) {
        ...
        if (
          workInProgress[14] !== null &&
          workInProgress[14].cachePool !== null
        ) {
          cache = workInProgress[14].cachePool.pool;
        }
        if (cache !== previousCache) {
          // Run passive effects to retain/release the cache.
          workInProgress[17] |= Passive;
